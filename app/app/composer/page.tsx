			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
				...
				<div className="flex items-center gap-3">
					<div className="relative">
						...
						{showScheduler && (
							<div className="absolute right-0 top-full mt-3 w-80 bg-background border border-border rounded-4xl p-6 z-50 shadow-2xl animate-in fade-in slide-in-from-top-2">
			...
			<div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
				...
				<div className="lg:col-span-7 space-y-8">
					...
					<div className="card-warm p-2 relative group">
						...
						<div className="flex items-center justify-between p-6 border-t border-border/30 bg-muted/10 rounded-b-3xl">
							...
							{content.length > minLimit && (
								<div className="flex items-center gap-2 text-red-500 text-[10px] font-bold uppercase tracking-widest animate-in fade-in duration-300">
									...
									{selectedPlatforms
										.filter(
											(p) =>
												PLATFORMS.find((pl) => pl.id === p)?.limit === minLimit,
										)
										.join(", ")}
								</div>
							)}
					...
					<div className="space-y-8 max-h-[70vh] overflow-auto pr-2 pb-10 scrollbar-hide">
						{selectedPlatforms.map((platformId) => {
							...
							return (
								<div
									key={platformId}
									className="bg-background border border-border/50 rounded-[2.5rem] shadow-lg overflow-hidden group transition-all hover:border-primary/20"
								>
									...
									<div className="p-8">
										<div className="flex items-start gap-4">
											<div className="w-12 h-12 rounded-full bg-secondary/20 shrink-0 flex items-center justify-center text-secondary font-bold border border-secondary/10">
												K
											</div>
											<div className="space-y-2 flex-1">
												...
												<p
													className={cn(
														...
														!content &&
															"text-muted-foreground/30 italic font-normal",
													)}
												>
