      ...
      <header className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
        <div>
          ...
          <p className="mt-3 text-[14px] text-ink/65 max-w-xl leading-[1.55]">
            Launches, webinars, sales, drips. Tell Muse the arc; get a
      ...
      {campaigns.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-border-strong bg-background-elev px-8 py-16 text-center">
          ...
          <p className="mt-2 text-[13.5px] text-ink/60 max-w-md mx-auto leading-[1.55]">
            A campaign is a sequenced arc of posts around one goal —
            perfect for product launches, webinars, seasonal sales, or
            evergreen drips.
          </p>
          <Link
            href="/app/campaigns/new"
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {campaigns.map((c) => {
            ...
            const accepted = (
              c.beats as Array<{ accepted?: boolean }>
            ).filter((b) => b.accepted).length;
            return (
              <li key={c.id}>
                <Link
