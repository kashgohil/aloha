...
export const metadata = makeMetadata({
	title: "Voice foundations — train and tune Muse",
	description:
		"A six-lesson learning path for Muse, Aloha's voice model. Goes from 'what is it' to 'I trust it with Wednesday's post' in roughly 45 minutes.",
	path: GUIDE_PATH,
});
...
const LESSONS = [
	{
		n: 1,
		title: "What Muse actually does",
		minutes: "4 min",
		summary:
			"Muse is Aloha's voice model. Why we trained it on 12 of your best posts instead of 1,200, and the 'sounds like me' loop in simple terms.",
		points: [
	...
	return (
		<>
			...
			<header className="relative overflow-hidden bg-peach-200 pb-16 lg:pb-20">
				...
				<span
					aria-hidden
					className="absolute top-[68%] right-[10%] font-display text-[22px] text-ink/15 rotate-14 select-none"
				>
					+
				</span>
			</header>
						...
						<div className="col-span-12 lg:col-span-8">
							<div className="flex flex-wrap items-center gap-3 mb-8 text-[11px] font-semibold uppercase tracking-[0.22em] text-ink/60">
								...
								<span className="px-2 py-0.5 bg-background-elev/70 border border-ink/10 rounded-full text-ink/65 text-[10px] font-mono normal-case tracking-normal inline-flex items-center gap-1.5">
									<Mic className="w-3 h-3" />6 lessons · {TOTAL_MINUTES}
								</span>
							</div>
							<h1 className="font-display font-normal text-ink leading-[0.95] tracking-[-0.03em] text-[56px] sm:text-[72px] lg:text-[88px]">
								...
								<span className="text-primary font-light">
									Trust it by Wednesday.
								</span>
							</h1>
							<p className="mt-8 max-w-2xl text-[17px] lg:text-[18px] leading-[1.6] text-ink/75">
								Six lessons, roughly 45 minutes total. By the end{" "}
								<span className="font-medium text-ink">Muse</span> — the voice
								model inside Composer — drafts in your cadence reliably enough
								that you'll hit Schedule on a real post without rereading three
								times.
							</p>
			...
			<section className="bg-ink relative">
				...
				/>
				<section className="py-12 lg:py-16 bg-background-elev wavy pb-32 lg:pb-40">
						...
						{LESSONS.map((l, idx) => (
							...
							>
								...
								<div className="col-span-12 md:col-span-3 p-7 lg:p-8 bg-muted/20 md:border-r border-border flex flex-col justify-between gap-6">
									<div>
										<span className="font-display text-[40px] lg:text-[48px] text-primary leading-none">
											0{l.n}
										</span>
									...
									<span
										className={`text-[10px] font-mono uppercase tracking-[0.18em] inline-flex items-center gap-1.5 ${idx === 0 ? "text-primary" : "text-ink/45"}`}
									>
								...
								<div className="col-span-12 md:col-span-9 p-7 lg:p-9">
									...
									<p className="mt-3 text-[15px] text-ink/75 leading-[1.6]">
										{l.summary}
									</p>
									...
									<ul className="mt-6 space-y-2.5 text-[14px] text-ink/85">
										{l.points.map((p) => (
											<li key={p} className="flex items-start gap-2.5">
												<ChevronRight
													className="w-3.5 h-3.5 mt-[5px] text-primary shrink-0"
													strokeWidth={2.5}
												/>
												{p}
											</li>
			...
			<section className="py-20 lg:py-24 bg-ink relative wavy text-background-elev">
				...
				/>
				<div className="max-w-[1180px] mx-auto px-6 lg:px-10 pb-12 lg:pb-20">
					<div className="relative grid grid-cols-12 gap-x-0 gap-y-8 lg:gap-8 items-center">
						<div className="col-span-12 lg:col-span-8">
							...
							<h2 className="font-display text-[28px] lg:text-[40px] leading-[1.15] tracking-[-0.015em]">
								Finished the path?
								<br />
								<span className="text-peach-300">Pick the next one.</span>
							</h2>
							<p className="mt-5 text-[14px] text-background-elev/75 leading-[1.6] max-w-xl">
								Calendar &amp; cadence picks up where this leaves off — how to
								put your now-trusted voice on a posting rhythm you can keep.
							</p>
						</div>
